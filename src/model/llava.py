...
from xtuner.utils.add_utils import add_special_tokens
...
class LLaVAModel(BaseModel):
    ...
    def __init__(
        ...
        if tokenizer_cfg is not None:
            self.tokenizer = BUILDER.build(tokenizer_cfg)
        else:
            self.tokenizer = None

        if add_tokens is not None:
            add_special_tokens(self.tokenizer, add_tokens)
            self.llm.resize_token_embeddings(len(self.tokenizer))
            self._initialize_new_tokens_weights(add_tokens)
        ...
        if pretrained_lora_path is not None:
            pretrained_state_dict = guess_load_checkpoint(pretrained_lora_path)

            self.load_state_dict(pretrained_state_dict, strict=False)
            print_log(f'Load pretrained lora weight from {pretrained_lora_path}', 'current')
    ...
    def _initialize_new_tokens_weights(self, add_tokens):
        # 获取词向量矩阵
        params = self.llm.get_input_embeddings().weight.data
        
        # 找到一个“安全”的参考 token，比如 "<" 或 "["
        # 这样新 token 的起点就在这些符号附近，而不是随机噪声
        reference_token_id = self.tokenizer.convert_tokens_to_ids("<")
        if reference_token_id == self.tokenizer.unk_token_id:
            reference_token_id = 0 # 保底方案
            
        reference_weight = params[reference_token_id].clone()
        
        # 获取所有新 token 的 ID
        # 这里假设 add_tokens 是你的新 token 列表
        for token in add_tokens:
            token_id = self.tokenizer.convert_tokens_to_ids(token)
            if token_id >= params.shape[0]: # 确保不越界
                continue
            params[token_id] = reference_weight.clone()
            print(f"Initialized weight for {token} (ID: {token_id}) with reference token ID: {reference_token_id}")
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        if 'pixel_values' in data:
            feat_to_proj = data['pixel_values'].to(self.llm.dtype) # [B, L, D]
            if self.enable_long_net:
                ...
                feat_to_proj = self.LongNet_encoder(src_tokens=None, token_embeddings=feat_to_proj)["encoder_out"] # [B, L, D]

            pixel_values = self.projector(feat_to_proj.to(self.llm.dtype)) # [B, L, llm_D]
    ...
    def to_xtuner_llava(self,
        ...
        if self.use_llm_lora:
            ...
            if not llm_merge:
                ...
                print_log(f'Saving merged LLM to {llm_path}', 'current')
                merged_llm = self.llm.merge_and_unload()
                merged_llm.save_pretrained(llm_path, **save_pretrained_kwargs)
        ...
        # 2. 手动保存权重字典
        torch.save(self.LongNet_encoder.state_dict(), os.path.join(LongNet_encoder_path, 'model.pth'))
        # 3. (可选) 如果原来的代码有保存配置的逻辑，这里可能无法保存 config，但权重是最重要的
        print(f"Manually saved LongNet_encoder to {LongNet_encoder_path}")
